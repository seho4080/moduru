...
public class PlaceInfoResolver {
    ...
    public void preload(List<WantPlace> wantPlaces) {
        // places 테이블의 id들
        Set<Long> placeIds = wantPlaces.stream()
                .filter(wp -> wp.getType() == PlaceType.PLACE)
                .map(WantPlace::getRefId)
                .collect(Collectors.toSet());

        // custom_places 테이블의 id들
        Set<Long> customIds = wantPlaces.stream()
                .filter(wp -> wp.getType() == PlaceType.CUSTOM)
                .map(WantPlace::getRefId)
                .collect(Collectors.toSet());

        placeMap = placeRepository.findAllWithImagesByIdIn(placeIds).stream()
                .collect(Collectors.toMap(Place::getId, Function.identity()));

        customMap = customPlaceRepository.findAllById(customIds).stream()
