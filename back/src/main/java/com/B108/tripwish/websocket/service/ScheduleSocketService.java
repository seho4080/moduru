...
public class ScheduleSocketService {
    ...
    public void updateSchedule(CustomUserDetails sender, Long roomId, ScheduleMessageRequestDto request) {
        Long userId = sender.getUser().getId();

        // 0. 이벤트에서 WantPlaceId 추출
        List<Long> wantIds = request.getEvents().stream()
        ...
        // 🔑 여기서 Map 생성
        Map<Long, WantPlace> wantPlaceMap = wantPlaces.stream()
        ...
        // 3. 응답 DTO 생성
        List<ScheduleEventMessageResponseDto> responseEvents = request.getEvents().stream()
        ...
        // 3. 원자적으로 버전 증가
        Long newVersion = redisTemplate.<String, Object>opsForHash()
                .increment("schedule:" + roomId + ":versions",
                        day.toString(),
        ...
        ScheduleMessageResponseDto response = ScheduleMessageResponseDto.builder()
    ...
    private boolean isStructuralChanged(DayScheduleRedisDto existing, List<ScheduleEventMessageRequestDto> newEvents) {
