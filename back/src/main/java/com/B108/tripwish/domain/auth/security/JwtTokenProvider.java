...
public class JwtTokenProvider {
    ...
    public JwtToken generateToken(Authentication authentication, String existingRefreshToken) {
        ...
        String accessToken = Jwts.builder().setSubject(authentication.getName()).claim("auth", authorities)
                .claim("type", TokenType.ACCESS.name()).setExpiration(accessTokenExpiresln)
        ...
        if (shouldIssueNewRefreshToken) {
            ...
            Date refreshTokenExpDate = Date.from(refreshTokenExpiresln.atZone(ZoneId.systemDefault()).toInstant());
            refreshToken = Jwts.builder().claim("type", TokenType.REFRESH.name()).setExpiration(refreshTokenExpDate)
                    .signWith(key, SignatureAlgorithm.HS256).compact();
        }

        return JwtToken.builder().grantType("Bearer").accessToken(accessToken).refreshToken(refreshToken)
                .refreshTokenExpiresAt(refreshTokenExpiresln).build();
    }
    ...
    public boolean validateToken(String token, TokenType type) {
        try {
            ...
            throw new CustomException(
                    type == TokenType.ACCESS ? ErrorCode.EXPIRED_ACCESS_TOKEN : ErrorCode.EXPIRED_REFRESH_TOKEN);

        } catch (SignatureException e) {
            throw new CustomException(type == TokenType.ACCESS ? ErrorCode.INVALID_ACCESS_SIGNATURE
                    : ErrorCode.INVALID_REFRESH_SIGNATURE);

        } catch (JwtException e) { // 그 외 파싱 문제 등
            throw new CustomException(
                    type == TokenType.ACCESS ? ErrorCode.INVALID_ACCESS_TOKEN : ErrorCode.INVALID_REFRESH_TOKEN);
        }
    }
