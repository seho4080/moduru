...
public class AuthController {
    ...
    @Operation(
            summary = "로그아웃",
            description = "현재 로그인한 사용자가 로그아웃을 시도합니다. Access Token을 기반으로 처리되며, 서버 측에서 해당 토큰을 무효화하거나 블랙리스트에 등록할 수 있습니다.",
            parameters = {
                    @Parameter(
                            name = "Authorization",
    ...
    @Operation(
            ...
            responses = {
                    ...
                    @ApiResponse(responseCode = "400", description = "유효하지 않은 Refresh Token 요청입니다.", content = @Content),
                    @ApiResponse(responseCode = "401", description = "Refresh Token이 만료되었거나 인증되지 않음", content = @Content),
                    @ApiResponse(responseCode = "500", description = "서버 오류", content = @Content)
            }
    )
    @PostMapping("/reissue")
