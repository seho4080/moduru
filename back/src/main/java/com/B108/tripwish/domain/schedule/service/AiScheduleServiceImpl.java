...
public class AiScheduleServiceImpl implements AiScheduleService {
    ...
    public String enqueue(Long roomId, AiScheduleRequestDto req) {
        if (req == null || req.getPlaceList() == null || req.getPlaceList().size() < 2 || req.getDays() < 1) {
    ...
    protected void runAsync(Long roomId, String jobId, AiScheduleRequestDto req) {
        try {
            ...
            var ids = spec.getPlaceList() == null ? List.<Long>of()
                    : spec.getPlaceList().stream().map(AiScheduleSpec.AiPlaceInfoDto::getId).toList();
            log.info("[🔎AI-SCHEDULE] ▶ Spec summary roomId={} days={} places={} ids={}",
                    roomId, spec.getDays(), placeCnt, ids);
            ...
            List<Long> aiWantIds = result.stream()
            ...
            Map<Long, WantPlace> wantById = loaded.stream()
                    .collect(Collectors.toMap(WantPlace::getId, w -> w));
    ...
    private AiRecommendBroadcastDto buildPreviewPayload(
            Long roomId,
            List<AiScheduleResult> ai,
            Map<Long, WantPlace> wantById
    ) {
        List<AiRecommendBroadcastDto.ScheduleInfo> schedule = ai.stream()
                .sorted(Comparator.comparingInt(AiScheduleResult::getDay))
                .map(day -> {

                    List<AiRecommendBroadcastDto.RouteDto> legs = Optional.ofNullable(day.getRoute())
                            .orElseGet(List::of)
                            .stream()
    ...
    public AiSnapshotResponseDto getRoomSnapshot(Long roomId) {
        ...
        // 3) 응답 DTO 빌더 생성
        AiSnapshotResponseDto.AiSnapshotResponseDtoBuilder builder = AiSnapshotResponseDto.builder()
                .roomId(roomId)
                .status(type)
                .jobId(jobId)
                .progress(progress)
        ...
        // 4) DONE 상태면 결과도 붙이기
        if ("DONE".equals(type)) {
            redisAiService.getRoomResult(roomId).ifPresent(resultJson -> {
