...
public class PlaceController {
    ...
    @Operation(
            ...
            description = "장소 검색 탭의 지역 기반 기본 장소 목록을 카테고리별(all, restaurant, cafe, attraction, stay, etc)로 조회합니다.",
            responses = {
                    ...
                    @ApiResponse(responseCode = "404", description = "존재하지 않는 roomId이거나 해당 방의 장소가 없음", content = @Content),
    ...
    @Operation(
            ...
            responses = {
                    ...
                    @ApiResponse(responseCode = "400", description = "검색어(keyword)가 누락되었거나 잘못된 요청입니다.", content = @Content),
                    @ApiResponse(responseCode = "404", description = "해당 roomId 또는 검색 조건에 해당하는 장소가 없습니다.", content = @Content),
                    @ApiResponse(responseCode = "500", description = "서버 내부 오류", content = @Content)
            }
    )
    ...
    @Operation(
            ...
            description = "사용자가 입력한 자연어 검색어를 기반으로 AI가 장소를 분석하여 반환합니다. " +
                    "해당 검색은 방 ID에 연결된 여행 지역 내에서만 수행됩니다.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "장소 검색 결과 조회 성공"),
                    @ApiResponse(responseCode = "400", description = "검색어(keyword)가 누락되었거나 잘못된 요청입니다.", content = @Content),
                    @ApiResponse(responseCode = "404", description = "해당 roomId 또는 검색 조건에 해당하는 장소가 없습니다.", content = @Content),
    ...
    @Operation(
            ...
            responses = {
                    ...
                    @ApiResponse(responseCode = "404", description = "해당 장소의 세부정보를 찾을 수 없음", content = @Content),
                    @ApiResponse(responseCode = "500", description = "서버 내부 오류", content = @Content)
            }
    )
    @GetMapping("/detail")
