...
public class AiRouteServiceImpl implements AiRouteService {
    ...
    public String enqueue(Long roomId, AiRouteRequestDto spec) {
        if (spec == null || spec.getPlaceList() == null || spec.getPlaceList().size() < 2 || spec.getDay() < 1) {
    ...
    protected void runAsync(Long roomId, String jobId, AiRouteRequestDto req) {
        try {
            ...
            log.info("[AI-ROUTE] ▶ result roomId={} day={}",
                    roomId, result.getDay());

            // AI 반환 ID 검증 + 메타 로드
            List<Long> aiWantIds = result.getRoute().stream()
                    .map(AiRouteResult.RouteInfo::getId)
                    .filter(Objects::nonNull)
                    .distinct()
                    .toList();

            List<WantPlace> loaded = aiWantIds.isEmpty()
                    ? List.of()
                    : wantPlaceRepository.findAllById(aiWantIds);
            ...
            Map<Long, WantPlace> wantById = loaded.stream()
    ...
    protected AiRouteSpec buildSpec(Long roomId, AiRouteRequestDto req) {
        // 1) 원본 순서 유지 + 중복 제거
        List<Long> wantIds = req.getPlaceList().stream()
                .filter(Objects::nonNull)
                .distinct()
                .toList();

        // 만약 placeList가 List<AiRouteRequestDto.AiPlaceInfoDto> 라면 ↑ 대신 아래 한 줄:
        // List<Long> wantIds = req.getPlaceList().stream().map(AiRouteRequestDto.AiPlaceInfoDto::getId).filter(Objects::nonNull).distinct().toList();
        ...
        Map<Long, WantPlace> byId = places.stream()
                .collect(Collectors.toMap(WantPlace::getId, Function.identity()));
    ...
    private AiRecommendBroadcastDto buildPreviewPayload(
            Long roomId,
            AiRouteResult result,
            Map<Long, WantPlace> wantById
    ) {
        List<AiRecommendBroadcastDto.RouteDto> legs = result.getRoute().stream()
    ...
    public AiRouteSnapshotResponseDto getRouteSnapshot(Long roomId) {
        ...
        if (statusJsonOpt.isEmpty()) {
            return AiRouteSnapshotResponseDto.builder()
                    .roomId(roomId).status("IDLE").updatedAt(Instant.now()).build();
        }
        ...
        var builder = AiRouteSnapshotResponseDto.builder()
                .roomId(roomId).status(type).jobId(jobId)
                .progress(progress).message(message).reason(reason).updatedAt(updatedAt);

        if ("DONE".equals(type)) {
            redisAiService.getRoomResult(AiTaskType.ROUTE, roomId)
    ...
    /**
     Redis에 저장되어 있는 JSON 문자열을 안전하게 꺼내고,
     DTO에 맞는 타입으로 변환하기 위한 헬퍼 메서드
     */
    ...
    private Map<String, Object> parseJson(String json) {
        try {
            ...
            return Map.of("type", "ERROR", "message", "snapshot-parse-failed", "updatedAt", Instant.now().toString());
        }
    }
    ...
    private Instant asInstant(Object o) {
        ...
        try { return Instant.parse(o.toString()); } catch (Exception e) { return Instant.now(); }
    }
