...
public class ScheduleController {
    ...
    public ResponseEntity<List<ScheduleListResponseDto>> getSchedule(@AuthenticationPrincipal CustomUserDetails user,
    ...
    """,
            parameters = {
                    @Parameter(name = "roomId", description = "여행 방 ID", required = true)
            },
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    required = true,
                    description = "여행 전체 커밋 요청(각 day별 draftVersion 포함)",
                    content = @Content(schema = @Schema(implementation = ScheduleCommitRequestDto.class))
            ),
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            ...
                            content = @Content(
                                    array = @ArraySchema(schema = @Schema(implementation = ScheduleListResponseDto.class))
                            )
                    ),
                    @ApiResponse(
                            responseCode = "409",
                            description = "버전 충돌: 서버 최신 일정/버전 반환",
                            content = @Content(schema = @Schema(implementation = ScheduleConflictResponseDto.class))
                    ),
                    @ApiResponse(responseCode = "400", description = "저장할 draft 없음(SCHEDULE_EMPTY_DRAFT)", content = @Content),
                    @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자", content = @Content),
    ...
    public ResponseEntity<?> commit(@PathVariable Long roomId,
                                    @RequestBody ScheduleCommitRequestDto request) {
        try {
