...
public class GoogleDirectionsClient {
    ...
    /** 대중교통 전용 호출 */
    public Mono<String> transit(String origin, String destination, String waypoints, Instant departureTime) {
        return directionsJson(origin, destination, waypoints, "transit", departureTime);
    }

    /**
     * Directions API v1(JSON) — walking / transit 전용
     * - 순서 고정: waypoints에 optimize:true 쓰지 말 것
    ...
    public Mono<String> directionsJson(
        ...
        String base = UriComponentsBuilder.fromPath("/directions/json")
        ...
        if ("transit".equals(m)) {
            ...
            if (departureTime != null) {
                b.queryParam("departure_time", departureTime.getEpochSecond()); // ← UTC epoch seconds
            } else {
                b.queryParam("departure_time", "now");
            }
            // 필요 시 선호 옵션도 추가 가능:
            // b.queryParam("transit_routing_preference", "fewer_transfers"); // or "less_walking"
        }
    ...
    public record LatLng(double lat, double lng) {
        public String asString() { return lat + "," + lng; }
    }
