...
# Ollama 클라이언트 초기화
client = Client(host="http://localhost:11434")  # 기본 포트


def summarize_korean_text(text, num_sentences=5):
    prompt = f"""다음 텍스트를 {num_sentences}문장으로 한국어로 요약해줘:

{text}
"""

    response = client.chat(model="gemma:7b", messages=[{"role": "user", "content": prompt}])
...
if __name__ == "__main__":
    ...
    summary = summarize_korean_text(long_korean_text, num_sentences=3)
